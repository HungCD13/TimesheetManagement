...
const ShiftChangeService = require('../services/shiftChangeService');

const createRequest = async (req, res) => {
  try {
    const request = await ShiftChangeService.createRequest(
      req.user,
      req.body
    );
    res.status(201).json(request);
...
const approveRequest = async (req, res) => {
  try {
    const request = await ShiftChangeService.approveRequest(
      req.user,
      req.params.id,
      req.body.status
    );
...
const getRequests = async (req, res) => {
  try {
    const requests = await ShiftChangeService.getRequests(req.user);
    res.json(requests);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
